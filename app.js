...
class FourWallShadowCutter {
    constructor() {
        ...
        this.tLabels = []; 
        this.tControls = null; // 新增控制变量，方便进行容错
    ...
    initThreeJS() {
        ...
        // 防御性编程：检测 OrbitControls 是否因为 CDN 被墙而加载失败
        if (typeof THREE.OrbitControls !== 'undefined') {
            this.tControls = new THREE.OrbitControls(this.tCamera, this.tRenderer.domElement);
            this.tControls.enableDamping = true;
            this.tControls.dampingFactor = 0.05;
        } else {
            console.warn("OrbitControls 脚本未能加载。已自动禁用 3D 旋转控制，但不影响图纸生成。");
        }
        ...
        const animate = () => {
            ...
            if (this.tControls) this.tControls.update(); // 容错处理
    ...
    updateThreeJSBox() {
        if (!this.tBoxGroup || this.tPlanes.length === 0) return;
        const boxW = parseFloat(document.getElementById('box-width').value);
        const boxH = parseFloat(document.getElementById('box-height').value);
        const boxD = parseFloat(document.getElementById('box-depth').value);
        ...
        for(let i=0; i<4; i++) {
            if(this.tPlanes[i] && this.tPlanes[i].material) {
    ...
    applyTexturesToThreeJS() {
        if (!this.tPlanes || this.tPlanes.length === 0) return; // 容错处理
        
        for (let i = 0; i < 4; i++) {
            if (!this.wallTextures[i] || !this.tPlanes[i]) continue;
    ...
    showPostCalculation3D() {
        if (!this.binaryImageData || !this.tActualShadow) return;
